...
func replaceContainers(c *cli.Context) error {
	...
	pod, entry, image, network, nodes, _, _, _, _, _, _, envs, count, _, _, files, user, debug, _, _, ignoreHook, afterCreate, _ := getDeployParams(c) // nolint
	if entry == "" || image == "" {
	...
	deployOpts := generateDeployOpts(data, pod, entry, image, network, nodes, 0, 0, 0, 0, 0, 0, envs, count, nil, "", files, user, debug, false, ignoreHook, 0, afterCreate, "")
