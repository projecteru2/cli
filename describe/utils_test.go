...
func TestToResourcePercent(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			usage:    `{"resource-storage":{"storage":50,"volumes":{"/data":10,"/tmp":10}}}`,
			capacity: `{"resource-storage":{"storage":200,"volumes":{"/data":40,"/tmp":40}}}`,
			wantCPU:  map[string]float64{},
			wantSt:   map[string]float64{"storage": 0.25, "volumes": 0.25},
		},
		{
			name:     "no volume capacity keeps zero percent",
			usage:    `{"resource-storage":{"storage":50,"volumes":{}}}`,
			capacity: `{"resource-storage":{"storage":200,"volumes":{}}}`,
			wantCPU:  map[string]float64{},
