...
build: ## Build eru-cli binary
	CGO_ENABLED=0 go build -ldflags "$(GO_LDFLAGS)" -o eru-cli .
...
fmt: gofumpt goimports ## Format code with gofumpt and goimports
	$(GOFMT) -extra -l -w .
	$(GOIMPORTS) -l -w --local 'github.com/projecteru2/cli' .
...
clean: ## Remove build artifacts, coverage files, and test cache
	rm -f eru-cli eru-cli-linux-* eru-cli-darwin-*
	rm -rf bin/ dist/
	rm -f coverage.out coverage.html coverage.txt
	go clean -testcache

cloc: ## Count lines of code excluding tests (requires cloc)
	cloc --exclude-dir=dist --not-match-f='_test\.go$$' .

# --- Help ---

help: ## Show this help message
	@echo "eru-cli Makefile targets:"
	@echo ""
